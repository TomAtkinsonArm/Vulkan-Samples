...
bool BenchmarkMode::is_active(const vkb::Parser &parser)
{
	return parser.contains(benchmark_flag);
}

void BenchmarkMode::init(const vkb::Parser &parser, vkb::OptionalProperties *properties)
{
	// Whilst in benchmark mode fix the fps so that seperate runs are consistently simulated
	// This will effect the graph outputs of framerate
	properties->render_properties.fixed_simulation_fps = 60.0f;
}
