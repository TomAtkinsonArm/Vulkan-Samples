...
GlfwWindow::GlfwWindow(Platform *platform, const Extent &extent, const WindowProperties &properties) :
    Window(extent, properties)
{
	if (!glfwInit())
	{
	...
	switch (properties.mode)
	{
		...
		default:
			handle = glfwCreateWindow(extent.width, extent.height, properties.title.c_str(), NULL, NULL);
			break;
	}
