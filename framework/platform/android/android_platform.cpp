...
void AndroidPlatform::create_window()
{
	...
	auto *options = get_plugin<::plugins::WindowOptions>();
	window        = std::make_unique<AndroidWindow>(*this, Window::WindowMode::Default, app->window, options->get_window_mode() == vkb::Window::WindowMode::Headless);
}
